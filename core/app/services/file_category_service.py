...
from app.utils.error_handler import FileManagementError, ErrorCode, ErrorDetail
        ...
        if not cls.validate_category(new_category):
            raise FileManagementError(
                ErrorDetail(
                    ...
                    user_message=f"Please select a valid category: {', '.join(cls.VALID_CATEGORIES)}",
                    http_status=400,
                    context={'valid_categories': cls.VALID_CATEGORIES}
                )
            )
        ...
        if not file_obj:
            raise FileManagementError(
                ErrorDetail(
                    ...
                    user_message="File not found or you don't have access",
                    http_status=404,
        ...
        if not success:
            raise FileManagementError(
                ErrorDetail(
                    ...
                    user_message="Failed to update file category",
                    http_status=400,
                    context={'file_id': file_id, 'category': new_category}
                )
            )
        ...
        try:
            ...
            raise FileManagementError(
                ErrorDetail(
                    ...
                    user_message="Failed to save category update",
                    http_status=500,
                    context={'error': str(e)}
                )
            )
        ...
        if not file_ids:
            raise FileManagementError(
                ErrorDetail(
                    ...
                    user_message="Please select at least one file",
                    http_status=400,
        ...
        try:
            ...
            raise FileManagementError(
                ErrorDetail(
                    ...
                    user_message="Failed to perform bulk category update",
                    http_status=500,
                    context={'error': str(e)}
                )
            )
        ...
        if category and category != 'all' and not cls.validate_category(category):
            raise FileManagementError(
                ErrorDetail(
                    ...
                    user_message=f"Please select a valid category: {', '.join(cls.VALID_CATEGORIES + ['all'])}",
                    http_status=400,
                    context={'valid_categories': cls.VALID_CATEGORIES + ['all']}
                )
            )
        ...
        try:
            ...
            raise FileManagementError(
                ErrorDetail(
                    ...
                    user_message="Failed to retrieve files",
                    http_status=500,
                    context={'error': str(e)}
                )
            )
        ...
        try:
            ...
            raise FileManagementError(
                ErrorDetail(
                    ...
                    user_message="Failed to retrieve category statistics",
                    http_status=500,
                    context={'error': str(e)}
                )
            )
